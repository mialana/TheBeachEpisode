    ...
    if (GetCharacterMovement()->IsFalling())
    {
        ...
        if (!bHasWallJumped)
        {
                                                 ...
                                                 ECollisionChannel::ECC_Visibility, TraceShape, QueryParams))
            {
                ...
                const FVector WallJumpImpulse = (OutHit.ImpactNormal * WallJumpBounceImpulse)
...
void APlatformingCharacter::DoDash()
{
    ...
    if (UAnimInstance* AnimInstance = GetMesh()->GetAnimInstance())
    {
        ...
        if (MontageLength > 0.0f) AnimInstance->Montage_SetEndDelegate(OnDashMontageEnded, DashMontage);
    }
}
...
void APlatformingCharacter::DashMontageEnded(UAnimMontage* Montage, bool bInterrupted)
{
    ...
    if (bInterrupted && bIsDashing) return;
