...
void ACombatCharacter::DoChargedAttackStart()
{
    ...
    if (bIsAttacking)
    {
        ...
        if (!bHasLoopedChargedAttack) bHasReleasedChargedAttack = false;
                                           ...
                                           QueryParams))
    {
        ...
        for (const FHitResult& CurrentHit : OutHits)
        {
            ...
            if (Damageable)
            {
                ...
                const FVector Impulse = (CurrentHit.ImpactNormal * -MeleeKnockbackImpulse)
...
void ACombatCharacter::LoopOrResolveChargedAttack()
{
    ...
    if (UAnimInstance* AnimInstance = GetMesh()->GetAnimInstance())
    {
        AnimInstance->Montage_JumpToSection(bHasReleasedChargedAttack ? ChargeAttackSection : ChargeLoopSection,
